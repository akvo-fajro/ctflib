...
# oracle func : input a message(bytes) and output (message + plain)'s cipher(bytes)
def prepend_orcale_attack(oracle):
    plainlength = len(oracle(b''))
    plain = b''
    print('')
    print('',b'attacking : ',end='')
    for j in range(plainlength):
        correct = oracle(b'a'*(plainlength-1-j))[:plainlength]
        for i in range(256):
            test = b'a'*(plainlength-1-j)+plain+bytes([i])
            plaintest = oracle(test)[:plainlength]
...
# input orginal (iv + cipher)(bytes) and oracle func
# output the plain(bytes) of padding oracle server
# oracle func : input a (iv + cipher)(bytes) and output a boolean
#               the boolean mean (iv + cipher)'s plain's padding is/not correct
def padding_oracle_attack(cipher, oracle):
...
def coppersmith_method(beta,epsilon,N,f):
    delta = f.degree()
    X = int(N**(beta**2 / delta - epsilon)) + 1
...
# input mbar(the know message)(int) , c(int) , e(int) , N(int)
...
def stereotyped_message(mbar,c,e,N):
    ...
    f = (mbar + x)**e - c
    smallroot = coppersmith_method(1,1/8,N,f)
    return smallroot
...
# input n1,n2
# if in [n1,n2) has just one int , output that int
# else output -1
def check_between_int(n1,n2):
    if ceil(n1) + 1 == ceil(n2):
        return int(ceil(n1))
    return -1
    

# it's with LSB oracle attack (just write for more beauty)
def binary_search(seq,n,e,oracle):
    [n1,n2,c] = seq
    if check_between_int(n1,n2) != -1:
        return [check_between_int(n1,n2)]*2 + [c]
...
# input n , e , c(int) , oracle func
# output m(int)
# oracle func : intput c(int) and output lowwest bit of cipher's plain(int)
def LSB_oracle_attack(n,e,c,oracle):
    seq = [0,n,c]
    while True:
        seq = binary_search(seq,n,e,oracle)
        if seq[0] == seq[1]:
            return seq[0]
...
# input n (int), e (int), c1 (int), c2 (int), f (polynomial) (f(m1) = m2)
# output m1 (int)
...
def coppersmith_short_pad_attack(n,c1,c2,e):
    ...
    diff = int(h.small_roots(epsilon=1/Integer(30))[0])
